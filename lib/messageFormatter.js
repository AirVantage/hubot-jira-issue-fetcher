...
function buildDetailedMessage(issue) {
  return {
    ...
    title_link: `https://${config.JIRA_HOST}/browse/${issue.key}`,
    text: issue.fields.description,
    fields: [{ title: 'Status', value: issue.fields.status.name }],
  };
}
...
function buildMessage(issue) {
  return {
    ...
    author_name: issue.fields.assignee ? issue.fields.assignee.displayName : 'Unassigned',
    author_icon: issue.fields.assignee ? issue.fields.assignee.avatarUrls['16x16'] : null,
    text: `<https://${config.JIRA_HOST}/browse/${issue.key}|${issue.key}> \`${issue.fields.status.name}\` ${issue.fields.summary}`,
    mrkdwn_in: ['text'],
  };
}
